...
pub fn create_g_funcs_comparison_graphs(num_repetitions: usize, num_iters: usize, population_size: usize)
{
    ...
    for g_index in 0..g_fitnesses.len() {
        ...
        plot_multiple(&avg_fitness_stats, &method_names, &TAB_COLORS, fitness_plot_filename.as_str(), fitness_plot_name.as_str(), log_opt_value, "Log avg. fitness", true).unwrap();
        plot_multiple(&avg_constraints_stats, &method_names, &TAB_COLORS, constraints_plot_filename.as_str(), constraints_plot_name.as_str(), log_1, "Log avg. constraints sum", true).unwrap();
